...
bool check(char *word)
{
    node *cursor = root;  // Apuntar al root original
    int N = strlen(word); // Variable con el tamaño de la string
    for (int i = 0; i < N; i++)
    {
        int index = tolower(word[i]) - 'a'; // El index de la letra en el abecedario ingles
        if (cursor->children[index] ==
            NULL) // Si no existe un nombre inicializado en la trie empezando con esa letra, devuelve false
        {
            return false;
        }
        else
        {
            cursor = cursor->children[index]; // Si existe, movemos el cursor a la siguiente array.
        }
    }
    return true; // Si no hemos encontrado un NULL, devolvemos True
}
